...
// Registers the Instagram subscription to receive updates whenever one of the authenticated users posts a media.
// ??? Why is this called for every registration, when one subscription is for all authentication app users ???
function registerIGSubscription(cb) {
	request.post('https://api.instagram.com/v1/subscriptions/', {
		form: {
			...
			object: 'user',
			aspect: 'media',
			verify_token: auth.instagramAuth.verifyToken,
	...
	function(error, response) {
		if (error || response.statusCode !== 200) {
			...
			if (responseBody && responseBody.meta && responseBody.meta.error_message) {
				errorMessage += responseBody.meta.error_message;
			} else {
				errorMessage += response.body;
			}

			console.log('Error in Instagram subscription registration => ' + errorMessage);
		} else {
			console.log('Instagram subscription registered successfully.');
		}
